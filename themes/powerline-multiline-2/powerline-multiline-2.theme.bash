...
function powerline_shell_prompt {
    ...
    if [[ "${SEGMENT_AT_LEFT}" -gt 0 ]]; then
      SHELL_PROMPT=$(set_rgb_color ${LAST_THEME_COLOR} ${SHELL_PROMPT_COLOR})${THEME_PROMPT_SEPARATOR}${normal}${SHELL_PROMPT}
    fi
    LEFT_PROMPT_LENGTH=$(( ${LEFT_PROMPT_LEFT} + ${#SHELL_PROMPT} + 2 ))
...
function powerline_in_vim_prompt {
  if [ -z "$VIMRUNTIME" ]; then
    ...
    IN_VIM_PROMPT="$(set_rgb_color - ${IN_VIM_PROMPT_COLOR}) ${IN_VIM_PROMPT_TEXT} "
      ...
      #(( RIGHT_PROMPT_LENGTH += SEGMENT_AT_RIGHT ))
    #fi
    LEFT_PROMPT_LENGTH=$(( ${LEFT_PROMPT_LENGTH} + ${#IN_VIM_PROMPT_TEXT} ))
    LAST_THEME_COLOR=${IN_VIM_PROMPT_COLOR}
    (( SEGMENT_AT_LEFT += 1 ))
    (( LEFT_PROMPT_LENGTH += 1 ))
  fi
}
...
function powerline_prompt_command() {
    ...
    LEFT_PROMPT="${IN_VIM_PROMPT}${SHELL_PROMPT}${SCM_PROMPT}${VIRTUALENV_PROMPT}${RVM_PROMPT}${CWD_PROMPT}${MOVE_CURSOR_RIGHTMOST}"
    ...
    #RIGHT_PROMPT+="${IN_VIM_PROMPT}${CLOCK_PROMPT}${BATTERY_PROMPT}${normal}"
    RIGHT_PROMPT=''
