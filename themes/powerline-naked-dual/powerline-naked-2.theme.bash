...
SCM_THEME_PROMPT_CLEAN_COLOR=78
...
function powerline_shell_prompt {
    if [[ -n "${SSH_CLIENT}" ]]; then
      ...
      SHELL_PROMPT="$(set_rgb_color ${SHELL_THEME_PROMPT_COLOR} -) \u ${normal}"
    fi
}
...
function powerline_last_status_prompt {
    if [[ "$1" -eq 0 ]]; then
        ...
        LAST_STATUS_PROMPT="$(set_rgb_color ${LAST_STATUS_THEME_PROMPT_COLOR} -) ${LAST_STATUS} ${normal}${PROMPT_CHAR}"
    fi
}
...
function powerline_prompt_command() {
    ...
    PS1="${SHELL_PROMPT}${VIRTUALENV_PROMPT}${SCM_PROMPT}${CWD_PROMPT}${THEME_PROMPT_SEPARATOR}\n ${LAST_STATUS_PROMPT} "
}
